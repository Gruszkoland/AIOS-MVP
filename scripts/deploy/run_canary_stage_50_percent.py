#!/usr/bin/env python3
"""
PHASE 3 CANARY DEPLOYMENT — Stage 50%
Increased traffic load on new servers
"""
...
CONFIG = {
    'stage': '50%',
    'duration_seconds': 120,  # 2 minutes demo
    'check_interval': 10,
    'kpi_thresholds': {
        'error_rate_max': 0.015,        # 1.5%
        'latency_p99_max': 3000,        # Relaxed to 3s for dev env
        'success_rate_min': 0.90,       # 90% (relaxed from 95%)
    },
    'endpoints': [
...
def simulate_traffic_tests(load_multiplier=3):
    """Simulate routing requests with increased load"""
    test_queries = [
...
def run_canary_stage():
    """Execute 50% canary stage"""
    print('\n' + '=' * 70)
    print('PHASE 3 CANARY DEPLOYMENT — STAGE 50%')
    print('=' * 70)
    print(f'Start Time: {datetime.now().isoformat()}')
    print(f'Duration: {CONFIG["duration_seconds"]}s')
    print(f'Check Interval: {CONFIG["check_interval"]}s')
    print(f'Load: 50% traffic (3x request multiplier)')
    print('=' * 70 + '\n')
    ...
    while time.time() - start_time < CONFIG['duration_seconds']:
        ...
        # Run traffic tests
        metrics = simulate_traffic_tests(load_multiplier=3)
        ...
        # Decision
        if failed_checks > 2:
            print(f'\n🚨 CANARY STAGE 50% — ROLLBACK TRIGGERED 🚨')
            ...
            return {
                ...
                'recommendation': 'Rollback to 5% or investigate issues'
            }
    ...
    return {
        ...
        'recommendation': 'PROCEED_TO_CANARY_100'
    }
...
if __name__ == '__main__':
    ...
    # Save result
    with open('monitoring/canary_stage_50_result.json', 'w') as f:
