"""
Unified Admin Panel (UAP) — Database Integration (SQLite + PostgreSQL)
Fallback to SQLite for local development if PostgreSQL unavailable.
...
# Detect DB engine from environment
DB_ENGINE = os.getenv("DB_ENGINE", "sqlite").lower()
DB_PATH = os.getenv("DB_PATH", "./db/adrion_local.db")

# PostgreSQL config (for when engine is postgres)
...
logger.info(f"[DB] Using engine: {DB_ENGINE}")
...
class SQLiteDB:
    ...
    def _init_schema(self):
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS genesis_logs (
                ...
                guards_passed INTEGER DEFAULT 9,
                notes TEXT,
    ...
    def export_agent_metrics(self) -> List[Dict[str, Any]]:
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute("""
                SELECT agent, trust_score, pleasure, arousal, dominance, timestamp
                FROM agent_metrics ORDER BY timestamp DESC
            """)
            rows = cursor.fetchall()
            return [
                {
...
# ============================================================================
# Factory Function: Select appropriate database engine
# ============================================================================
def get_database_engine():
    """Factory function to get the appropriate database engine."""
    if DB_ENGINE == "sqlite":
        logger.info("[DB] Using SQLite backend")
        return SQLiteDB()
    elif DB_ENGINE == "postgres" and PostgresDB:
        logger.info("[DB] Using PostgreSQL backend")
        return PostgresDB()
    else:
        logger.info("[DB] PostgreSQL not available or not selected, falling back to SQLite")
        return SQLiteDB()
