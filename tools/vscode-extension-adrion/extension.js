...
function activate(context) {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider(
      AdrionViewProvider.viewType,
      provider,
    ),
  );

  context.subscriptions.push(
    vscode.commands.registerCommand("adrion.runTask", async (taskLabel) => {
  ...
  // New command for kubectl operations
  context.subscriptions.push(
    vscode.commands.registerCommand("adrion.runKubectl", async (command, isLive = false) => {
      const terminal = vscode.window.createTerminal("ADRION K8s");
      terminal.show();

      if (isLive) {
        // For live logs, add a note
...
class AdrionViewProvider {
  static viewType = "adrion-control-view";
...
<body>
    ...
    <button class="btn" onclick="runKubectlCommand('curl -H \"X-API-Key: local-dev-key-123\" http://localhost:8002/mapi/v1/status', false, true)">✅ Test Backend API</button>
    <button class="btn" onclick="runKubectlCommand('curl http://localhost:8003', false, true)">✅ Test Frontend</button>
    ...
    <script>
        ...
        function runKubectlCommand(command, isLive = false, isRemote = false) {
            let taskName;
            if (isRemote) {
                // For curl/http commands, show in terminal
                taskName = command;
            } else if (isLive) {
                // For live logs
                taskName = command;
            } else {
                // For normal kubectl commands
                taskName = command;
            }
